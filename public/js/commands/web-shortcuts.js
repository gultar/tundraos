...
function runLinguee(params=[]){
    ...
    createWindow({ title: "Linguee", height:"100%", width:"100%", html:`
    <iframe 
    id="wiki-window" 
    style="border:none;" 
...
function runLofi(){
    if(window.isElectron) launchBrowser('https://www.youtube.com/watch?v=jfKfPfyJRdk&ab_channel=LofiGirl')
        ...
        createWindow({ title: "Lofi Girl", height:"90%", width:"90%", html:lofi });
    }
...
function runMap(){
    ...
    createWindow({ title: "Google Maps", height:"95%", width:"80%", html:maps  });
}
